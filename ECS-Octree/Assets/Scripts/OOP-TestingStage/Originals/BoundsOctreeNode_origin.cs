...
public class BoundsOctreeNode<T> {
	...
	const int numObjectsAllowed = 64;
...
Debug.Log ( "Node: Remove #" + children[i].GetHashCode () ) ;
GameObject.Destroy ( GameObject.Find ( children[i].GetHashCode ().ToString () ) ) ;
	...
	public bool IsColliding(ref Bounds checkBounds, out int i_instanceIndex ) 
    {
		...
		for (int i = 0; i < objects.Count; i++) {
            ...
            {
                i_instanceIndex = int.Parse ( objects[i].Obj.ToString ()  ) ;
				return true;
			}
		}
		...
		if (children != null) {
			for (int i = 0; i < 8; i++) {
				if (children[i].IsColliding(ref checkBounds, out i_instanceIndex)) {
					return true ;
				}
			}
		}
	...
	/// <returns>True if there was a collision.</returns>
	public bool IsColliding (ref Ray checkRay, out int i_instanceIndex, float maxDistance = float.PositiveInfinity) {
		...
		if (children != null) {
			for (int i = 0; i < 8; i++) {
				if (children[i].IsColliding ( ref checkRay, out i_instanceIndex, maxDistance)) 
                {
                    i_instanceIndex = -1 ;
					return true;
				}
