...
Debug.Log ( "Node: New game object #" + rootNode.GetHashCode () ) ;
	...
	public bool IsColliding(Bounds checkBounds, out int i_instanceIndex ) {
		//#if UNITY_EDITOR
		// For debugging
		//AddCollisionCheck(checkBounds);
		//#endif
		return rootNode.IsColliding ( ref checkBounds, out i_instanceIndex );
	}
	...
	/// <returns>True if there was a collision.</returns>
	public bool IsColliding(Ray checkRay, out int i_instanceIndex, float maxDistance ) {
		//#if UNITY_EDITOR
		// For debugging
		//AddCollisionCheck(checkRay);
		//#endif
		return rootNode.IsColliding (ref checkRay, out i_instanceIndex , maxDistance);
	}
